...
const CACHE_NAME = 'perseverantia-v4-font-clean';
const RUNTIME_CACHE = 'perseverantia-runtime-v4-font-clean';
const STATIC_ASSETS = [
  '/',
  '/index.html',
  '/events.html',
  ...
  if (request.destination === 'style' || request.destination === 'script' || request.destination === 'font') {
    ...
    event.respondWith(
      caches.match(request).then((cachedResponse) => {
        ...
        // Return a basic cached response if available
        return caches.match(request);
      })
    );
  } else if (request.method === 'GET' && (request.destination === 'image' || request.destination === '')) {
