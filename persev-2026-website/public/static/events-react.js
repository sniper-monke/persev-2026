...
function App() {
  ...
  const openEventRop = useCallback((event) => {
    if (event && event.ropLink) {
      window.open(event.ropLink, '_blank', 'noopener');
    }
  }, []);
  ...
  useEffect(() => {
    if (isFilterOpen) {
      ...
      filterTimerRef.current = setTimeout(() => setIsFilterOpen(false), 2000);
    }
    return () => {
      if (filterTimerRef.current) clearTimeout(filterTimerRef.current);
    };
  ...
  return html`
    <div className="events-react-root">
      ...
      <section className="selected-work fade-in-up" aria-labelledby="selected-events-title">
        ...
        <div className="ev-filter-drawer">
          <button
            type="button"
            className=${'ev-filter-toggle' + (isFilterOpen ? ' is-open' : '')}
            aria-label="Toggle category filter"
            onClick=${() => setIsFilterOpen((p) => !p)}
          >
            <svg class="ev-filter-toggle__icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
              <polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/>
            </svg>
            <span class="ev-filter-toggle__label">Filters</span>
            <span class="ev-filter-toggle__dot"></span>
          </button>
          <div className=${'ev-filter-drawer__panel' + (isFilterOpen ? ' is-open' : '')}>
            <${EventCategoryPanel}
              events=${allEvents}
        ...
        <div className="active-showcase" aria-live="polite">
          ...
          <div className="active-showcase__meta">
            ...
            ${activeEvent?.ropLink
              ? html`<a href=${activeEvent.ropLink} target="_blank" rel="noopener" className="active-showcase__cta" id="activeEventButton">
                  View ROP <span aria-hidden="true">→</span>
                </a>`
              : html`<span className="active-showcase__cta active-showcase__cta--disabled">
                  View ROP
                </span>`}
          </div>
        </div>
